...
void memoryModify(state *prgmState);
...
void loadFile(state *prgmState)
{
	...
	printf("Please enter location in file to start loading from (hex): ");
	fgets(tmpBuf, BUF_SIZE, stdin);
	sscanf(tmpBuf, "%x", &location);

	printf("Please enter amount of units to load (dec): ");
	fgets(tmpBuf, BUF_SIZE, stdin);
...
void memoryDisplay(state *prgmState)
{
	...
	printf("Please enter amount of units to display (dec): ");
	fgets(tmpBuf, BUF_SIZE, stdin);
	sscanf(tmpBuf, "%d", &u);

	printf("Please enter address of memory to display (hex): ");
	fgets(tmpBuf, BUF_SIZE, stdin);
...
void saveIntoFile(state *prgmState)
{
	...
	printf("Please enter address of memory to write from (hex): ");
	fgets(tmpBuf, BUF_SIZE, stdin);
	sscanf(tmpBuf, "%p", &addr);

	printf("Please enter location to write at in file (hex): ");
	fgets(tmpBuf, BUF_SIZE, stdin);
	sscanf(tmpBuf, "%x", &location);

	printf("Please enter amount of units to write (dec): ");
	fgets(tmpBuf, BUF_SIZE, stdin);
	sscanf(tmpBuf, "%d", &length);
...
void memoryModify(state *prgmState)
{
	...
	printf("Please enter location to write in the buffer (hex): ");
	fgets(tmpBuf, BUF_SIZE, stdin);
	sscanf(tmpBuf, "%x", &location);

	printf("Please enter value to insert (hex, unit size: %d): ", prgmState->unit_size);
	fgets(tmpBuf, BUF_SIZE, stdin);
	sscanf(tmpBuf, "%x", &val);

	printDebug(prgmState, "Location: %X\nValue: %X\n", location, val);
	if(location > prgmState->mem_count || location < 0){
		printf("Memory Modify: location out of bounds\n");
		return;
	}
	memmove(prgmState->mem_buf + location, &val, prgmState->unit_size);
}
