...
//antivirus functions
VIRUS *readVirus(FILE *virusSigs);
...
int main(int argc, char **argv)
{
	...
	FUN_DESC menu[] = {{"Load virusSigs", &loadSignatures},
					  {"Print virusSigs", &printSignatures},
...
LINK *loadSignatures(LINK *virusList)
{
	...
	virusSigs = fopen(filename, "r");

	while (virusSigs != NULL && fgetc(virusSigs) != EOF)
	{
		fseek(virusSigs, -1, SEEK_CUR);
		curVirus = readVirus(virusSigs);
...
{
	char input[BUF_SIZE], inputTmp[BUF_SIZE];
	int signatureOffset = 0, signatureSize = 0;

	printf("Insert virus location in file: ");
	fgets(input, BUF_SIZE, stdin);
	sscanf(input, "%s", inputTmp);
	signatureOffset = atoi(inputTmp);

	printf("Insert virus size: ");
	fgets(input, BUF_SIZE, stdin);
	sscanf(input, "%s", inputTmp);
	signatureSize = atoi(inputTmp);

	kill_virus(file_to_check, signatureOffset, signatureSize);
...
void kill_virus(char *fileName, int signitureOffset, int signitureSize)
{
	...
	if (file != NULL)
	{
		...
		fwrite(buffer, sizeof(char), signitureSize, file);
		fclose(file);
	}
	else{
		printf("Could not open file.\n");
